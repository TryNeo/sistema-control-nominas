<?php
    ...
    class EmpleadosModel extends Mysql{
        ...
        public function selectEmpleados(){
            ...
            empl.sueldo,puest.momnre_puesto,cont.nombre_contracto,empl.estado FROM empleados as empl 
            INNER JOIN contractos as cont ON empl.id_contracto = cont.id_contracto INNER JOIN puestos as puest ON empl.id_puesto = puest.id_puesto";
        ...
        string $str_telefono,float $float_sueldo,int $int_id_puesto,int $int_id_contracto,int $int_estado)
        {
            $return = 0;
            ...
            $request = $this->select_sql_all($sql);
            if(empty($request)){
                $sql_insert = "INSERT INTO empleados (nombre,apellido,cedula,telefono,sueldo,id_puesto,id_contracto,estado,fecha_crea) values (?,?,?,?,?,?,?,?,now())";
